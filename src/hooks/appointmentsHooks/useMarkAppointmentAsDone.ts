...
export const useMarkAppointmentAsDone = () => {
    const markAsDone = useCallback(
      async (appointmentId: string, isDone: boolean, user: string, comment: string): Promise<Appointment | null> => {
        try {
          const updatedAppointment = await markAppointmentAsDone(appointmentId, isDone, user, comment);
